...
Leman.el is an independent fork of [[https://github.com/alphapapa/ement.el][Ement.el]], a [[http://www.matrix.org/][Matrix]] client for [[https://www.gnu.org/software/emacs/][GNU Emacs]].  It aims to be simple, fast, featureful, and reliable, while integrating naturally with Emacs.
...
The default formatting style resembles IRC clients, with each message being prefixed by the username (which enables powerful Emacs features, like using Occur to show all messages from or mentioning a user).  Alternative, built-in styles include an Elleman-like one with usernames above groups of messages, as well as a classic, no-margins IRC style.  Messages may be optionally displayed with unique colors for each user (with customizable contrast), making it easier to follow conversations.  Timestamp headers are optionally displayed where a certain amount of time passes between events, as well as where the date changes.

[[images/leman-for-twim.png]]
...
Leman is not published in any package archive; install it from Git, e.g. with [[https://github.com/quelpa/quelpa-use-package][quelpa-use-package]], like this (using [[https://github.com/alphapapa/unpackaged.el#upgrade-a-quelpa-use-package-forms-package][this helpful command]] for upgrading versions):
...
#+BEGIN_SRC elisp
  ...
  ;; Install Leman.
  (use-package leman
    :quelpa (leman :fetcher github :repo "thargosu/ement.el"))
#+END_SRC
...
Leman.el is intended to be installed with Emacs's package system, which will ensure that the required autoloads are generated, etc.  If you choose to install it manually, you're on your own.
...
1. Call command ~leman-connect~ to connect.  Multiple sessions are supported: call the command again with a ~C-u~ universal prefix to connect to another account.
...
3. Use these commands (room-related commands may be called with universal prefix to prompt for the room):
   - ~leman-list-rooms~ to view the list of joined rooms.
   - ~leman-view-room~ to view a room's buffer, selected with completion.
   - ~leman-create-room~ to create a new room.
   - ~leman-create-space~ to create a space.
   - ~leman-invite-user~ to invite a user to a room.
   - ~leman-join-room~ to join a room.
   - ~leman-leave-room~ to leave a room.
   - ~leman-forget-room~ to forget a room.
   - ~leman-tag-room~ to toggle a tag on a room (including favorite/low-priority status).
   - ~leman-list-members~ to list members in a room.
   - ~leman-send-direct-message~ to send a direct message to a user (in an existing direct room, or creating a new one automatically).
   - ~leman-room-edit-message~ to edit a message at point.
   - ~leman-room-send-file~ to send a file.
   - ~leman-room-send-image~ to send an image.
   - ~leman-room-set-topic~ to set a room's topic.
   - ~leman-room-occur~ to search in a room's known events.
   - ~leman-room-override-name~ to override a room's display name.
   - ~leman-ignore-user~ to ignore a user (or with interactive prefix, un-ignore).
   - ~leman-room-set-message-format~ to set a room's message format buffer-locally.
   - ~leman-room-toggle-space~ to toggle a room's membership in a space (a way to group rooms in Matrix).
   - ~leman-directory~ to view a room directory.
   - ~leman-directory-search~ to search a room directory.
4. Use these special buffers to see events from multiple rooms (you can also reply to messages from these buffers!):
   - See all new events that mention you in the =*Leman Mentions*= buffer.
   - See all new events in rooms that have open buffers in the =*Leman Notifications*= buffer.
...
+ Write message: ~RET~
+ Compose message in buffer: ~M-RET~ (while writing in minibuffer: ~C-c '‍~).  Customize the option ~leman-room-compose-method~ to make ~RET~ and the other message bindings use a compose buffer by default.  Use command ~leman-room-compose-org~ to activate Org mode in the compose buffer.
...
+ Send messages in Org mode format by customizing the option ~leman-room-send-message-filter~ (which enables Org format by default), or by calling ~leman-room-compose-org~ in a compose buffer (which enables it for a single message).  Then Org-formatted messages are automatically converted and sent as HTML-formatted messages (with the Org syntax as the plain-text fallback).  You can send syntax such as:
...
+ Customize ~leman-room-use-variable-pitch~ to render messages using proportional fonts.
+ You can customize settings in the ~leman~ group.
...
Leman.el doesn't support encrypted rooms natively, but it can be used transparently with the E2EE-aware reverse proxy daemon [[https://github.com/matrix-org/pantalaimon/][Pantalaimon]].  After configuring it according to its documentation, call ~leman-connect~ with the appropriate hostname and port, like:

#+BEGIN_SRC elisp
  (leman-connect :uri-prefix "http://localhost:8009")
#+END_SRC
...
+ Support Matrix spoilers in formatted messages (as sent by, e.g. Element's ~/spoiler~ command).  Spoiled content is hidden behind a ~[spoiler]~ label (which includes the spoiler's reason, when given); clicking the label or pressing ~RET~ on it reveals or hides the content.
+ Support Matrix threads (MSC3440).  Thread replies are not shown in the main timeline; thread roots show a summary chip (~🧵 N~ followed by the latest reply) which opens a thread view buffer (also with ~s t~ with point on the root or a reply).  In the thread view, ~RET~ replies to the thread, ~g~ refreshes it from the server, and ~q~ quits.  Thread summaries are shown with the new ~%T~ message format specifier (included in the default formats).

*Fixes*

+ Video, audio, and file messages are formatted robustly when their metadata is incomplete (e.g. older ~m.file~ events, which have no ~filename~).

** 0.17

*Additions*

+ Command ~leman-room-download-file~, which downloads the file in the event at point (for image, audio, video, and file messages).  ([[https://github.com/alphapapa/ement.el/pull/323][#323]].  Thanks to [[https://github.com/viiru-][Arto Jantunen]].)
+ Customization groups for faces.  (Thanks to [[https://github.com/phil-s][Phil Sainty]].)
+ Option ~leman-room-hide-redacted-message-content~, which hides the content of redacted messages by default.  It may be disabled to keep redacted content visible with a strikethrough face, which may be useful for room moderators, but users should keep in mind that doing so will leave unpleasant content visible in the current session, even after being redacted by moderators.
+ Option ~leman-room-list-avatar-generation~: if disabled, SVG-based room avatars are not generated.  This option automatically tests whether SVG support is available in Emacs, and should allow use with builds of Emacs that lack =librsvg= support. 

*Changes*

+ Disable underline for faces ~leman-room-list-direct~ and ~leman-room-list-name~ (in case a face they inherit from enables it, e.g. when themed).

*Fixes*

+ Call ~eww-browse-url~ instead of ~browse-url~ in ~leman-room-browse-mxc~ (because the latter is not useful for authenticated media if the user has configured it to use a different browser).  ([[https://github.com/alphapapa/ement.el/pull/323][#323]].  Thanks to [[https://github.com/viiru-][Arto Jantunen]].)
...
+ When option ~leman-room-images~ is disabled (preventing automatic download and display of images), individual images may be shown by clicking the button in their events.

*Changes*

+ Option ~leman-room-coalesce-events~ may now be set to (and defaults to) a maximum number of events to coalesce together.  (This avoids potential performance problems in rare cases.  See [[https://github.com/alphapapa/ement.el/issues/247][#247]].  Thanks to [[https://github.com/viiru-][Arto Jantunen]] for reporting and [[https://github.com/sergiodj][Sergio Durigan Junior]] for testing.)
...
+ Command ~leman-disconnect~ no longer shows an error message.  ([[https://github.com/alphapapa/ement.el/issues/208][#208]].)
...
+ Customization group for options ~leman-room-mode-hook~ and ~leman-room-self-insert-mode~.  (Thanks to [[https://github.com/phil-s][Phil Sainty]].)
...
*Fixes*
+ Handle unnamed rooms in ~leman-directory~ list.  (See [[https://github.com/alphapapa/ement.el/issues/248][#248]].  Thanks to [[https://github.com/hjozwiak][Hunter Jozwiak]] and [[https://github.com/bmp][Bharath Palavalli]] for reporting.)
...
+ Configurable emoji picker for sending reactions.  ([[https://github.com/alphapapa/ement.el/issues/199][#199]], [[https://github.com/alphapapa/ement.el/pull/201][#201]].  Thanks to [[https://github.com/oantolin][Omar Antolín Camarena]].) ::
  - Option ~leman-room-reaction-picker~ sets the default picker.  Within that, the user may press ~C-g~ to choose a different one with a key bound in ~leman-room-reaction-map~.
...
+ Global minor mode ~leman-room-self-insert-mode~ enables "just typing" to start a message.  (Thanks to [[https://github.com/phil-s][Phil Sainty]].) :: See [[#leman-room-self-insert-mode][leman-room-self-insert-mode]].
...
+ Aborted messages are now added to ~leman-room-message-history~ rather than the kill-ring.  (Thanks to [[https://github.com/phil-s][Phil Sainty]].)
+ Prefix bindings in ~leman-room-mode-map~ now have named labels in ~which-key~ and similar.  (Thanks to [[https://github.com/phil-s][Phil Sainty]].)
+ Option: ~leman-room-use-variable-pitch~ (previously named ~leman-room-shr-use-fonts~) enables variable-pitch fonts for all message types.  (This option previously supported formatted messages, but now works for plain text messages as well.)  Note: users who have customized the ~leman-room-message-text~ face to be variable-pitch should revert that change, as it causes problems for formatted messages, and is no longer necessary.  ([[https://github.com/alphapapa/ement.el/issues/174][#174]].  Thanks to [[https://github.com/phil-s][Phil Sainty]].)
...
+ Newlines in ~leman-room-message-format-spec~ are considered when calculating the wrap-prefix.  (Thanks to [[https://github.com/phil-s][Phil Sainty]].)
+ Weight of face ~leman-room-list-direct~ (now correctly bold in room list heading).
...
- Option ~leman-room-compose-buffer-display-action~ declares how and where a new compose buffer window should be displayed.  (By default, in a new window below the associated room buffer.)
- Option ~leman-room-compose-buffer-window-dedicated~ determines whether compose buffers will have dedicated windows.
- Option ~leman-room-compose-buffer-window-auto-height~ causes dynamic scaling of the compose buffer window height so that the full message is visible at all times.
- Option ~leman-room-compose-buffer-window-auto-height-min~ specifies the minimum window height when ~leman-room-compose-buffer-window-auto-height~ is enabled.
- Option ~leman-room-compose-buffer-window-auto-height-max~ specifies the maximum window height when ~leman-room-compose-buffer-window-auto-height~ is enabled.
- Option ~leman-room-compose-method~ chooses between minibuffer-centric or compose-buffer-centric behaviour.
- Command ~leman-room-dispatch-new-message~ starts writing a new message using your chosen ~leman-room-compose-method~.  (Bound to ~RET~ in room buffers.)
- Command ~leman-room-dispatch-new-message-alt~ starts writing a new message using the alternative method.  (Bound to ~M-RET~ in room buffers.)
- Command ~leman-room-dispatch-edit-message~ edits a message using your chosen ~leman-room-compose-method~.  (Bound to ~<insert>~ in room buffers.)
- Command ~leman-room-dispatch-reply-to-message~ replies to a message using your chosen ~leman-room-compose-method~.  (Bound to ~S-<return>~ in room buffers.)
- Command ~leman-room-compose-edit~ edits a message using a compose buffer.
- Command ~leman-room-compose-reply~ replies to a message using a compose buffer.
- Command ~leman-room-compose-send-direct~ sends a message directly from a compose buffer (without the minibuffer).  (Bound to ~C-x C-s~ in compose buffers.)
- Command ~leman-room-compose-abort~ kills the compose buffer and delete its window.  (Bound to ~C-c C-k~ in compose buffers.)
- Command ~leman-room-compose-abort-no-history~ does the same without adding to ~leman-room-message-history~.  (Equivalent to ~C-u C-c C-k~.)
- Command ~leman-room-compose-history-prev-message~ cycles backwards through ~leman-room-message-history~.  (Bound to ~M-p~ in compose buffers.)
- Command ~leman-room-compose-history-next-message~ cycles forwards through ~leman-room-message-history~.  (Bound to ~M-n~ in compose buffers.)
- Command ~leman-room-compose-history-isearch-backward~ initiates an isearch through ~leman-room-message-history~.  (Bound to ~M-r~ in compose buffers; continue searching with ~C-r~ or ~C-s~.)
- Command ~leman-room-compose-history-isearch-backward-regexp~ initiates a regexp isearch through ~leman-room-message-history~.  (Bound to ~C-M-r~ in compose buffers; continue searching with ~C-r~ or ~C-s~.)

*** ~leman-room-self-insert-mode~
:PROPERTIES:
:CUSTOM_ID: leman-room-self-insert-mode
:END:

- Option ~leman-room-self-insert-commands~ determines which commands will start a new message when ~leman-room-self-insert-mode~ is enabled (defaulting to ~self-insert-command~ and ~yank~).
- Option ~leman-room-self-insert-chars~ determines which typed characters will start a new message when ~leman-room-self-insert-mode~ is enabled (regardless of whether they are bound to ~self-insert-command~).
- Option ~leman-room-mode-map-prefix-key~ defines a prefix key for accessing the full ~leman-room-mode-map~ when ~leman-room-self-insert-mode~ is enabled.  (By default this key is ~DEL~.)
...
- Option ~leman-room-image-margin~ is the number of pixels of margin around image thumbnails.
- Option ~leman-room-image-relief~ is the number of pixels of shadow rectangle around image thumbnails.
- Option ~leman-room-image-thumbnail-height~ is the window body height multiple to use when toggling full-sized images to thumbnails (by default, 0.2).
- Option ~leman-room-image-thumbnail-height-min~ is the minimum pixel height for thumbnail images (by default, 30 pixels).
...
+ Customization group ~leman-room-list~.
+ Option ~leman-room-list-space-prefix~ is applied to space names in the room list (e.g. set to empty string for cleaner appearance).
+ Option ~leman-room-reaction-names-limit~ sets how many senders of a reaction are shown in the buffer (more than that many are shown in the tooltip).
...
+ Command ~leman-room-send-reaction~ checks for an event at point.  (Thanks to [[https://github.com/phil-s][Phil Sainty]].)
...
+ Command ~leman-room-image-show~ use frame parameters to maximize the frame, making it easier for users to override.  ([[https://github.com/alphapapa/ement.el/issues/223][#223]].  Thanks to [[https://github.com/progfolio][Nicholas Vollmer]].)
...
+ Command ~leman-room-flush-colors~ maintains point position.

** 0.12

*Additions*

+ Command ~leman-notifications~ shows recent notifications, similar to the pane in the Element client.  (This new command fetches recent notifications from the server and allows scrolling up to retrieve older ones.  Newly received notifications, as configured in the ~leman-notify~ options, are displayed in the same buffer.  This functionality will be consolidated in the future.)
+ Face ~leman-room-quote~, applied to quoted parts of replies.

*Changes*
+ Commands ~leman-room-goto-next~ and ~leman-room-goto-prev~ work more usefully at the end of a room buffer.  (Now pressing ~n~ on the last event moves point to the end of the buffer so it will scroll automatically for new messages, and then pressing ~p~ skips over any read marker to the last event.)
+ Room buffer bindings:
  + ~leman-room-goto-next~ and ~leman-room-goto-prev~ are bound to ~n~ and ~p~, respectively.
  + ~leman-room-goto-fully-read-marker~ is bound to ~M-g M-p~ (the mnemonic being "go to previously read").
+ The quoted part of a reply now omits the face applied to the rest of the message, helping to distinguish them.
+ Commands that read a string from the minibuffer in ~leman-room~ buffers and ~leman-connect~ user ID prompts use separate history list variables.
...
+ Add workaround for ~savehist-mode~'s serializing of the ~command-history~ variable's arguments.  (For ~leman-~ commands, that may include large data structures, like ~leman-session~ structs, which should never be serialized or reused, and ~savehist~'s doing so could cause noticeable delays for users who enabled it).  (See [[https://github.com/alphapapa/ement.el/issues/216][#216]].  Thanks to [[https://github.com/phil-s][Phil Sainty]] and other users who helped to discover this problem.)

** 0.11

*Additions*
+ Commands ~leman-room-image-show~ and ~leman-room-image-scale~ (bound to ~RET~ and ~M-RET~ when point is at an image) view and scale images.  (Thanks to [[https://github.com/Stebalien][Steven Allen]] for these and other image-related improvements.)
+ Command ~leman-room-image-show-mouse~ is used to show an image with the mouse.

*Changes*
+ Enable ~image-mode~ when showing images in a new buffer.  (Thanks to [[https://github.com/Stebalien][Steven Allen]].)
+ Command ~leman-room-image-show~ is not used for mouse events.
...
+ Command ~leman-room-list-next-unread~ could enter an infinite loop.  (Thanks to [[https://github.com/vizs][Visuwesh]] and ~@mrtnmrtn:matrix.org~.)
...
*Internal*
+ The ~leman-read-receipt-idle-timer~ could be duplicated when using multiple sessions.  ([[https://github.com/alphapapa/ement.el/issues/196][#196]].  Thanks to [[https://github.com/phil-s][Phil Sainty]].)
...
+ Bind ~m~ in room buffers to ~leman-room-mark-read~ (which moves read markers to point).

*Changes*

+ Activating a space in the room list uses ~leman-view-space~ (which shows a directory of rooms in the space) instead of ~leman-view-room~ (which shows events in the space, which is generally not useful).
+ Command ~leman-view-room~, when used for a space, shows a footer explaining that the buffer is showing a space rather than a normal room, with a button to call ~leman-view-space~ for it (which lists rooms in the space).
+ Command ~leman-describe-room~ shows whether a room is a space or a normal room.
+ Command ~leman-view-space~ shows the space's name and alias.
+ Command ~leman-room-scroll-up-mark-read~ moves the fully read marker to the top of the window (when the marker's position is within the range of known events), rather than only moving it when at the end of the buffer.  (This eases the process of gradually reading a long backlog of messages.)
...
+ Don't reinitialize ~leman-room-list-mode~ when room list buffer is refreshed.  ([[https://github.com/alphapapa/ement.el/issues/146][#146]].  Thanks to [[https://github.com/treed][Ted Reed]] for reporting.)
...
+ Command ~leman-room-list-next-unread~.
...
*Fixes*
+ Error in ~leman-room-list~ command upon initial sync.

** 0.9

*Additions*

+ Option ~leman-room-timestamp-header-align~ controls how timestamp headers are aligned in room buffers.
+ Option ~leman-room-view-hook~ runs functions when ~leman-room-view~ is called.  (By default, it refreshes the room list buffer.)
...
+ Using new option ~leman-room-timestamp-header-align~, timestamp headers default to right-aligned.  (With default settings, this keeps them near message timestamps and makes for a cleaner appearance.)
...
+ Don't error in ~leman-room-list~ when no rooms are joined.  (Fixes [[https://github.com/alphapapa/ement.el/issues/123][#123]].  Thanks to [[https://github.com/Kabouik][@Kabouik]] and [[https://github.com/oantolin][Omar Antolín Camarena]] for reporting.)
...
*Additions*
+ Command ~leman-create-space~ creates a new space.
+ Command ~leman-room-toggle-space~ toggles a room's membership in a space (a way to group rooms in Matrix).
+ Visibility of sections in the room list is saved across sessions.
+ Command ~leman-room-list-kill-buffer~ kills a room's buffer from the room list.
+ Set ~device_id~ and ~initial_device_display_name~ upon login (e.g. =Leman.el: username@hostname=).  ([[https://github.com/alphapapa/ement.el/issues/134][#134]].  Thanks to [[https://github.com/viiru-][Arto Jantunen]] for reporting.)
...
+ Command ~leman-room-list~ reuses an existing window showing the room list when possible.  ([[https://github.com/alphapapa/ement.el/issues/131][#131]].  Thanks to [[https://github.com/jeffbowman][Jeff Bowman]] for suggesting.)
+ Command ~leman-tag-room~ toggles tags (rather than adding by default and removing when called with a prefix).
...
+ Prompts in commands ~leman-leave-room~ and ~leman-forget-room~.

** 0.7

*Additions*

+ Command ~leman-room-override-name~ sets a local override for a room's display name.  (Especially helpful for 1:1 rooms and bridged rooms.  See [[https://github.com/matrix-org/matrix-spec-proposals/pull/3015#issuecomment-1451017296][MSC3015]].)

*Changes*

+ Improve display of room tombstones (displayed at top and bottom of buffer, and new room ID is linked to join).
+ Use descriptive prompts in ~leman-leave-room~ and ~leman-forget-room~ commands.

*Fixes*

+ Command ~leman-view-space~ when called from a room buffer.  (Thanks to [[https://github.com/MagicRB][Richard Brežák]] for reporting.)
...
+ Function ~leman-put-account-data~ accepts ~:room~ argument to put on a room's account data.

** 0.6

*Additions*
+ Command ~leman-view-space~ to view a space's rooms in a directory buffer.

*Changes*
+ Improve ~leman-describe-room~ command (formatting, bindings).
...
*Fixes*
+ Apply ~leman-initial-sync-timeout~ properly (important for when the homeserver is slow to respond).

** 0.5.1

*Fixes*
+ Autoload ~leman-directory~ commands.
+ Faces in ~leman-directory~ listings.
...
*Changes*
+ The [[https://github.com/alphapapa/taxy.el][taxy.el]]-based room list, with programmable, smart grouping, is now the default ~leman-room-list~.  (The old, ~tabulated-list-mode~-based room list is available as ~leman-tabulated-room-list~.)
...
*Additions*
+ Option ~leman-room-unread-only-counts-notifications~, now enabled by default, causes rooms' unread status to be determined only by their notification counts (which are set by the server and depend on rooms' notification settings).
+ Command ~leman-room-set-notification-state~ sets a room's notification state (imitating Element's user-friendly presets).
+ Room buffers' Transient menus show the room's notification state (imitating Element's user-friendly presets).
+ Command ~leman-set-display-name~ sets the user's global displayname.
+ Command ~leman-room-set-display-name~ sets the user's displayname in a room (which is also now displayed in the room's Transient menu).
+ Column ~Notifications~ in the ~leman-taxy-room-list~ buffer shows rooms' notification state.
+ Option ~leman-interrupted-sync-hook~ allows customization of how sync interruptions are handled.  (Now, by default, a warning is displayed instead of merely a message.)
...
*Fixes*
+ Binding of command ~leman-forget-room~ in room buffers.
...
*Additions*
+ Command ~leman-directory~ shows a server's room directory.
+ Command ~leman-directory-search~ searches a server's room directory.
+ Command ~leman-directory-next~ fetches the next batch of rooms in a directory.
+ Command ~leman-leave-room~ accepts a ~FORCE-P~ argument (interactively, with prefix) to leave a room without prompting.
+ Command ~leman-forget-room~ accepts a ~FORCE-P~ argument (interactively, with prefix) to also leave the room, and to forget it without prompting.
+ Option ~leman-notify-mark-frame-urgent-predicates~ marks the frame as urgent when (by default) a message mentions the local user or "@room" and the message's room has an open buffer.

*Changes*
+ Minor improvements to date/time headers.

*Fixes*
+ Command ~leman-describe-room~ for rooms without topics.
...
+ The ~leman-taxy-room-list~ view no longer automatically refreshes the list if the region is active in the buffer.  (This allows the user to operate on multiple rooms without the contents of the buffer changing before completing the process.)
...
*Fixed*
+ Function ~leman-room-sync~ correctly updates room-list buffers.  (Thanks to [[https://github.com/vizs][Visuwesh]].)
...
*Fixed*
+ Function ~leman-room-scroll-up-mark-read~ selects the correct room window.
+ Option ~leman-room-list-avatars~ defaults to what function ~display-images-p~ returns.
...
/This section is preserved for posterity.  As it says, Leman.el has long since surpassed ~matrix-client~, which should no longer be used./
...
- ~matrix-client~ uses an older version of the Matrix spec, r0.3.0, with a few elements of r0.4.0 grafted in.  Bringing it up to date with the current version of the spec, r0.6.1, would be more work than to begin with the current version.  Leman.el targets r0.6.1 from the beginning.
- ~matrix-client~ does not use Matrix's lazy-loading feature (which was added to the specification later), so initial sync requests can take a long time for the server to process and can be large (sometimes tens of megabytes of JSON for the client to process!).  Leman.el uses lazy-loading, which significantly improves performance.
- ~matrix-client~ automatically makes buffers for every room a user has joined, even if the user doesn't currently want to watch a room.  Leman.el opens room buffers on-demand, improving performance by not having to insert events into buffers for rooms the user isn't watching.
- ~matrix-client~ was developed without the intention of publishing it to, e.g. MELPA or ELPA.  It has several dependencies, and its code does not always install or compile cleanly due to macro-expansion issues (apparently depending on the user's Emacs config).  Leman.el is designed to have minimal dependencies outside of Emacs (currently only one, ~plz~, which could be imported into the project), and every file is linted and compiles cleanly using [[https://github.com/alphapapa/makem.sh][makem.sh]].
- ~matrix-client~ uses EIEIO, probably unnecessarily, since few, if any, of the benefits of EIEIO are realized in it.  Leman.el uses structs instead.
- ~matrix-client~ uses bespoke code for inserting messages into buffers, which works pretty well, but has a few minor bugs which are difficult to track down.  Leman.el uses Emacs's built-in (and perhaps little-known) ~ewoc~ library, which makes it much simpler and more reliable to insert and update messages in buffers, and enables the development of advanced UI features more easily.
- ~matrix-client~ was, to a certain extent, designed to imitate other messaging apps.  The result is, at least when used with the ~matrix-client-frame~ command, fairly pleasing to use, but isn't especially "Emacsy."  Leman.el is intended to better fit into Emacs's paradigms.
- ~matrix-client~'s long name makes for long symbol names, which makes for tedious, verbose code.  ~leman~ is easy to type and makes for concise, readable code.
- The author has learned much since writing ~matrix-client~ and hopes to write simpler, more readable, more maintainable code in Leman.el.  It's hoped that this will enable others to contribute more easily.

Note that, while ~matrix-client~ remains usable, and probably will for some time to come, Leman.el has now surpassed it in every way.  The only reason to choose ~matrix-client~ instead is if one is using an older version of Emacs that isn't supported by Leman.el.
