;;; leman-macros.el --- Leman macros                 -*- lexical-binding: t; -*-
...
(cl-defmacro leman-debug (&rest args)
...
(defmacro leman-alist (&rest pairs)
...
(defmacro leman-afirst (form list)
...
(defmacro leman-aprog1 (first &rest body)
...
(defmacro leman-singly (place-form &rest body)
...
(defalias 'leman-progress-update #'ignore
  "By default, this function does nothing.  But inside
`leman-with-progress-reporter', it's bound to a function that
updates the current progress reporter.")

(defmacro leman-with-progress-reporter (args &rest body)
...
Around BODY, the function `leman-progress-update' is set to a
  ...
  (pcase-let* ((progress-reporter-sym (gensym))
               ...
               (update-fn `(cl-function
                            (lambda (&optional (value (cl-incf ,progress-value-sym)))
                              (leman-debug "Updating progress reporter to" value)
...
(cl-defmacro leman-with-room-and-session (&rest body)
  "Eval BODY with `leman-room' and `leman-session' bound.
If in an `leman-room-list-mode' buffer and `current-prefix-arg'
is nil, use the room and session at point.  If in an `leman-room'
buffer and `current-prefix-arg' is nil, use buffer-local value of
`leman-room' and `leman-session'.  Otherwise, prompt for them
with `leman-complete-room' or that given with :prompt-form.
...
BODY may begin with property list arguments, including:
  ...
  (pcase-let* ((plist (cl-loop while (keywordp (car body))
               ...
               (prompt-form (or (plist-get plist :prompt-form)
                                '(leman-complete-room :suggest t))))
    `(pcase-let* ((`[,list-room ,list-session] (if (eq 'leman-room-list-mode major-mode)
                                                   (oref (magit-current-section) value)
                                                 [nil nil]))
                  (leman-room (or list-room leman-room))
                  (leman-session (or list-session leman-session)))
       (when (or current-prefix-arg (not leman-room))
         (pcase-let ((`(,room ,session) ,prompt-form))
           (setf leman-room room
                 leman-session session)))
       ,@body)))

(defmacro leman-propertize (string &rest properties)
      ...
      (pcase (plist-get properties ''face #'equal)
        ...
        (_
         ...
         (let ((value-form (plist-get properties ''face #'equal))
               (value-var (gensym "leman-propertize-")))
...
(provide 'leman-macros)

;;; leman-macros.el ends here
